...
; of the next round.
  ...
  (io? prove nil state
       ...
       (pprogn
        ; TRACE-LOG[emit/forcing-round]: in :structured mode the forcing-round
        ; announcement must be a machine-parseable event, not English prose —
        ; the prose rides the prove channel (not inhibited) and previously
        ; LEAKED into the log, breaking the parser (mapping-arc pin
        ; cov-force-round, corrected 2026-07-23). :ROUND is the round about
        ; to BEGIN (r+1 — its clause-ids are [r+1]Goal/[r+1]Subgoal i),
        ; :GOALS the clause count poured into it, :ASSUMPTIONS the raw forced
        ; assumption count they were cleaned from. Logging-only.
        (cond
         ((eq (f-get-global 'raw-proof-format state) :structured)
          (fms "(:FORCING-ROUND :ROUND ~x0 :GOALS ~x1 :ASSUMPTIONS ~x2)~%"
