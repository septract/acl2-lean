...
; TRACE-LOG[infra/tp-leaves]: helper for the :TYPE-PRESCRIPTION proof data
; emitted by emit-structured-defuns (defuns.lisp) and
; emit-ground-zero-defuns (ld.lisp), and for the recognizer-site
; :ARG-LEAVES channel (rewrite-recognizer, rewrite.lisp) —
...
; TRACE-LOG[infra/tp-leaves]: (same collector family) the type-alist a
; stored type-prescription rule's OWN HYPOTHESES generate — each hyp
; ASSUMED TRUE through assume-true-false-rec, exactly as the IF walk above
; assumes a governing test.  This is what makes a CONDITIONAL rule's leaf
; verdicts computable: BINARY-APPEND's leaves are (3072, unknown) with no
; hypothesis but (1024, 1152) under TRUE-LISTP-APPEND's ((TRUE-LISTP B)),
; which is the datum an admissibility gate needs to accept the stored
; strengthening (T1+2 sprint, fork round-trip 2 ask 1).  A hypothesis set
; assume-true-false-rec finds MUST-BE-FALSE is ACL2's own contradictory
; context: VACUOUSP propagates and the leaves come back *ts-empty*, the
; same encoding the IF walk uses.  Returns (mv type-alist vacuousp).
(defun tp-hyps-type-alist (hyps type-alist vacuousp ens wrld)
  (declare (xargs :guard t :mode :program))
  (cond
   ((endp hyps) (mv type-alist vacuousp))
   (t (mv-let (must-be-true must-be-false true-type-alist false-type-alist
                            ttree)
        (assume-true-false-rec (car hyps)
                               nil    ; xttree
                               nil    ; force-flg
                               nil    ; dwp
                               type-alist
                               ...
                               nil    ; ignore0
                               nil)   ; backchain-limit
        (declare (ignore must-be-true false-type-alist ttree))
        (tp-hyps-type-alist (cdr hyps) true-type-alist
                            (or vacuousp (and must-be-false t))
                            ens wrld)))))
